...
xhr.onreadystatechange = function () {
  if (this.readyState == 4 && this.status == 200){
      ...
      {
        const prod = new category(data1[i].title, data1[i].image, data1[i].id, data1[i].price, data1[i].description, data1[i].category, data1[i].rating);
        if (clone == null)
          continue;
          ...
          if (id_prod_h1 && title_h3 && description_h6 && category_h3 && price_h3 && c_container)
          {
            ...
            img.src = prod.image;
            title_h3.innerHTML = prod.title
            description_h6.innerHTML = prod.description;
            category_h3.innerHTML = prod.category;
            price_h3.innerHTML = prod.price + "$";
